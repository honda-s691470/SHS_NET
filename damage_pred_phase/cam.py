...
class CAM_multi(object):
    """ Class Activation Mapping """

    def __init__(self, model, target_layer, multi_num):
        """
        Args:
            model: a base model to get CAM which have global pooling and fully connected layer.
            target_layer: conv_layer before Global Average Pooling
...
class ScoreCAM_multitask(CAM_multi):
    """ Score CAM """

    def __init__(self, model, target_layer, multi_num, n_batch=32):
        super().__init__(model, target_layer, multi_num)
        """
        Args:
            model: a base model
            target_layer: conv_layer you want to visualize
    ...
    def forward(self, x, idx=None):
        ...
        with torch.no_grad():
            ...
            self.model.zero_grad()
            score = self.model(x)
            ##########code revised for multitask#############
            prob = F.softmax(score[self.multi_num], dim=1)
            ...
            for i in range(0, C, self.n_batch):
                ...
                score = self.model(masked_x)
                ##########code revised for multitask#############
                probs.append(F.softmax(score[self.multi_num], dim=1)[:, idx].to('cpu').data)
