...
def validate_bestmodel(label_list_merge, score_pred_model1, score_pred_model2, score_pred_ensem, at_best_list, config, K_num, val_loader, model, pred_model1_list, pred_model2_list, pred_ensem_list, label_list, best_score_list, restart):
    ...
    if restart == 'on':
        ...
        pass      
    return label_list_merge, score_pred_model1, score_pred_model2, score_pred_ensem, best_score_list, K_num
...
def validate_bestmodel_restart(config, K_num, val_loader, model, pred_model1_list, pred_model2_list, pred_ensem_list, label_list):
    col_list = pd.read_csv(config['check_path'] + config['all_label']).columns
    model[0].load_state_dict(torch.load(config['check_path'] + 'damage_pred_log/models/' + config['name'] + f'/model{K_num}_1.pth'))
    model[0].eval()
    if config['mutual'] == 'yes': 
